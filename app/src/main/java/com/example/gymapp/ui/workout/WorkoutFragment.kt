...
class WorkoutFragment: Fragment(){
    ...
    override fun onCreateView(
        ...
        var adapter = WorkoutAdapter(emptyList(), args.weekCount, WorkoutAdapter.OnClickListener {
                workoutSet -> Toast.makeText(application, "$workoutSet", Toast.LENGTH_SHORT).show()
        })
        ...
        workoutViewModel.setList.observe(viewLifecycleOwner) {
            adapter = workoutViewModel.setList.value?.let { setList ->
                WorkoutAdapter(setList, args.weekCount, WorkoutAdapter.OnClickListener {
                    workoutSet -> Toast.makeText(application, "$workoutSet", Toast.LENGTH_SHORT).show()
                })
            }!!
            binding.setList.adapter = adapter
